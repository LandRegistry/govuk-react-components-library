...
import { NavigationItem } from "./ServiceNavigation.types";
...
describe("ServiceNavigation component", () => {
  ...
  test("should render basic ServiceNavigation properly", () => {
    render(
      <MemoryRouter>
        <ServiceNavigation
  ...
  Object.values(examplesFromFixtures).forEach((example) => {
    test(`Test Fixture for ServiceNavigation called "${example.name}"`, () => {
      render(
        <MemoryRouter>
          <ServiceNavigation {...example.options} />
        </MemoryRouter>,
      );
